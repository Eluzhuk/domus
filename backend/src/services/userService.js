...
const { email, password, role, scope, display_name } = payload;
...
`INSERT INTO users (email, password_hash, is_active, display_name)
	VALUES (:email, :hash, true, :display_name) RETURNING id`,
{ replacements: { email, hash, display_name: display_name || null } }
);
...
/**
 * Включает пользователя (is_active=true). Нельзя трогать superadmin.
 * Проверка: scope делегирующего ⊇ scope цели.
 * @param {number} grantorId
 * @param {number} targetUserId
 */
async function enableUser(grantorId, targetUserId) {
// Защитим superadmin
const [sr] = await sequelize.query(
	`SELECT 1
	FROM user_roles ur
...
/**
 * Обновляет профиль пользователя: email?, password?, display_name?
 * Проверки: scope (как при disable/enable).
 * @param {number} grantorId
 * @param {number} targetUserId
 * @param {{email?:string,password?:string,display_name?:string}} data
 */
async function updateProfile(grantorId, targetUserId, data) {
const grantorScope = await getUserScope(grantorId);
const roles = await getUserRolesWithScope(targetUserId);
for (const rr of roles) {
	if (!isScopeSubset(grantorScope, rr.scope || {})) throw new Error('SCOPE_FORBIDDEN');
...
if (data.email) {
	...
	const [ex] = await sequelize.query(
		...
		{ replacements: { email: data.email, id: targetUserId } }
	);
	if (ex?.length) throw new Error('EMAIL_EXISTS');
	await sequelize.query(
		`UPDATE users SET email = :email WHERE id = :id`,
		{ replacements: { email: data.email, id: targetUserId } }
	);
...
if (data.password) {
	...
	await sequelize.query(
		`UPDATE users SET password_hash = :hash WHERE id = :id`,
		{ replacements: { hash, id: targetUserId } }
	);
}

if (typeof data.display_name !== 'undefined') {
	await sequelize.query(
		`UPDATE users SET display_name = :dn WHERE id = :id`,
		{ replacements: { dn: data.display_name || null, id: targetUserId } }
	);
}
...
// Удаляем ВСЕ роли пользователя, кроме superadmin (на всякий случай),
// чтобы не копились admin+manager и прочие «хвосты».
await sequelize.query(
...
// Назначаем выбранную роль с новым scope
await sequelize.query(
	`INSERT INTO user_roles (user_id, role_id, scope)
	VALUES (:userId, :roleId, :scope::jsonb)
	ON CONFLICT (user_id, role_id) DO UPDATE SET scope = EXCLUDED.scope`,
